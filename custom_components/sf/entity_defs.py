...
def build_device_entities(
    ...
    if caps["hasFan"] and want("blower"):
        defs += [
            ...
            # ── Mode-aware controls (surfaced per-mode by the card's blower tile) ──
            d("select", "blower_mode_set", "Blower Mode Set", icon="mdi:cog",
              options=["Manual", "Time Slot", "Cycle", "Environment"],
              command_field="blower", command_subfield="mode"),
            d("select", "blower_run_mode", "Blower Run Mode", icon="mdi:cog-sync",
              options=["Prioritize temperature", "Prioritize humidity",
                       "Temperature only", "Humidity only",
                       "Temperature & humidity"],
              command_field="blower", command_subfield="env_submode"),
            d("text", "blower_schedule_start", "Blower Schedule Start",
              icon="mdi:clock-start", command_field="blower",
              command_subfield="schedule_start"),
            d("text", "blower_schedule_stop", "Blower Schedule Stop",
            ...
            d("number", "blower_standby_speed", "Blower Standby Speed", unit="%",
              ...
              command_field="blower", command_subfield="standby_speed"),
            d("text", "blower_cycle_start", "Blower Cycle Start",
              icon="mdi:clock-start", command_field="blower",
              command_subfield="cycle_start"),
            d("text", "blower_cycle_run", "Blower Cycle Run", kind="duration",
              icon="mdi:timer-play", command_field="blower",
              command_subfield="cycle_run"),
            d("text", "blower_cycle_off", "Blower Cycle Off", kind="duration",
              icon="mdi:timer-off", command_field="blower",
              command_subfield="cycle_off"),
            d("number", "blower_cycle_times", "Blower Cycle Times",
    ...
    for lf, ln, cf, blk in (("light_1", "Light 1", "light", "light"),
        ...
        defs += [
            ...
            d("text", f"{lf}_schedule_start", f"{ln} Schedule Start",
              ...
              command_field=cf, command_subfield="schedule_start"),
            d("text", f"{lf}_schedule_stop", f"{ln} Schedule Stop",
              icon="mdi:clock-end",
            ...
            d("text", f"{lf}_ppfd_start", f"{ln} PPFD Start",
              icon="mdi:clock-start",
              command_field=cf, command_subfield="ppfd_start"),
            d("text", f"{lf}_ppfd_stop", f"{ln} PPFD Stop",
              icon="mdi:clock-end",
              command_field=cf, command_subfield="ppfd_end"),
            d("number", f"{lf}_ppfd_fade", f"{ln} PPFD Fade", unit="min",
              icon="mdi:weather-sunset", num_mode="box",
    ...
    if dtype in _FULL_TYPES and want("humidifier"):
        defs += [
            ...
            # ── Mode-aware controls (card humidifier tile) ──
            ...
            d("text", "humidifier_schedule_start", "Humidifier Schedule Start",
              ...
              command_subfield="schedule_start"),
            d("text", "humidifier_schedule_stop", "Humidifier Schedule Stop",
              icon="mdi:clock-end", command_field="humidifier",
              command_subfield="schedule_end"),
            d("text", "humidifier_cycle_start", "Humidifier Cycle Start",
              icon="mdi:clock-start", command_field="humidifier",
              command_subfield="cycle_start"),
            d("text", "humidifier_cycle_run", "Humidifier Cycle Run", kind="duration",
              icon="mdi:timer-play", command_field="humidifier",
              command_subfield="cycle_run"),
            d("text", "humidifier_cycle_off", "Humidifier Cycle Off", kind="duration",
              icon="mdi:timer-off", command_field="humidifier",
              command_subfield="cycle_off"),
    ...
    if dtype in _FULL_TYPES and want("dehumidifier"):
        defs += [
            ...
            # ── Mode-aware controls (card dehumidifier tile) ──
            ...
            d("text", "dehumidifier_schedule_start", "Dehumidifier Schedule Start",
              ...
              command_subfield="schedule_start"),
            d("text", "dehumidifier_schedule_stop", "Dehumidifier Schedule Stop",
              icon="mdi:clock-end", command_field="dehumidifier",
              command_subfield="schedule_end"),
            d("text", "dehumidifier_cycle_start", "Dehumidifier Cycle Start",
              icon="mdi:clock-start", command_field="dehumidifier",
              command_subfield="cycle_start"),
            d("text", "dehumidifier_cycle_run", "Dehumidifier Cycle Run", kind="duration",
              icon="mdi:timer-play", command_field="dehumidifier",
              command_subfield="cycle_run"),
            d("text", "dehumidifier_cycle_off", "Dehumidifier Cycle Off", kind="duration",
              icon="mdi:timer-off", command_field="dehumidifier",
              command_subfield="cycle_off"),
    ...
    if dtype in _FULL_TYPES and want("heater"):
        defs += [
            ...
            # ── Mode-aware controls (card heater tile) ──
            ...
            d("text", "heater_schedule_start", "Heater Schedule Start",
              ...
              command_subfield="schedule_start"),
            d("text", "heater_schedule_stop", "Heater Schedule Stop",
              icon="mdi:clock-end", command_field="heater",
              command_subfield="schedule_end"),
            d("text", "heater_cycle_start", "Heater Cycle Start",
              icon="mdi:clock-start", command_field="heater",
              command_subfield="cycle_start"),
            d("text", "heater_cycle_run", "Heater Cycle Run", kind="duration",
              icon="mdi:timer-play", command_field="heater",
              command_subfield="cycle_run"),
            d("text", "heater_cycle_off", "Heater Cycle Off", kind="duration",
              icon="mdi:timer-off", command_field="heater",
              command_subfield="cycle_off"),
...
def build_air_calibration_entities(device_cfg, slot=None):
    ...
    (app convention; converted to degC on the wire); the rest are direct
    units. Writes go back via setConfigField ["calibration"]."""
    mac_raw = device_cfg.get("mac", "")
    mac = _mac(mac_raw)
    dname = _device_name(device_cfg)
    dmodel = _device_model(device_cfg)

    def num(field, name, unit, icon, lo, hi, step, mode="box"):
        return SfDef(
            platform="number", field=field, name=name, mac=mac, mac_raw=mac_raw,
            device_name=dname, device_model=dmodel, unit=unit, slot=slot,
            kind="cal", min_value=lo, max_value=hi, step=step, num_mode=mode,
...
def build_soil_calibration_entities(mac_raw, sensor_id, device_cfg, slot=None, soil_slot=None, include_substrate=True):
    """Per-probe editable calibration: 3 offset numbers
    (senConfig[].calibration tempSoil/humiSoil/ECSoil) + a Substrate select
    (senConfig[].soilType, Pro probes only). Soil-temp is entered in degF (app
    convention). Writes go back via setConfigField ["device","senConfig"]."""
    mac = _mac(mac_raw)
    ...
    def num(suff, label, unit, icon, lo, hi, step):
        return SfDef(
            platform="number", field=f"soil_{safe}_{suff}", name=f"{base} {label}",
            mac=mac, mac_raw=mac_raw, device_name=dname, device_model=dmodel,
            unit=unit, slot=slot, kind="cal", min_value=lo, max_value=hi, step=step,
    ...
    if include_substrate:  # Pro probes only — Basic probes have no substrate type
        out.append(SfDef(
            platform="select", field=f"soil_{safe}_substrate", name=f"{base} Substrate",
            mac=mac, mac_raw=mac_raw, device_name=dname, device_model=dmodel, slot=slot,
            options=list(SUBSTRATE_OPTIONS), entity_category="diagnostic",
            icon="mdi:layers-outline", object_id=oid("substrate"),
        ))
    return out
