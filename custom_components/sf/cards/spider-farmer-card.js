 ...
 * Copyright 2017 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */function ut(t){return pt({...t,state:!0,attribute:!1})}const ft=/^sf_(dp\d+|ac5|ac10)_/;function gt(t){return t.split(".")[1]??""}function $t(t){const e=new Set;for(const s of Object.keys(t.states)){const t=gt(s).match(ft);t&&e.add(t[1])}return[...e].sort()}function mt(t){return $t(t).filter(e=>Object.keys(t.states).some(t=>{const s=gt(t);return s===`sf_${e}_temperature`||s===`sf_${e}_soil_avg_temperature`||s===`sf_${e}_light_1`||s===`sf_${e}_fan`||s===`sf_${e}_blower`}))}function _t(t,e){const s=`sf_${e}_`,i=Object.keys(t.states);return i.find(t=>gt(t)===`sf_${e}_temperature`)??i.find(t=>{const i=gt(t);return i.startsWith(s)&&!i.startsWith(`sf_${e}_env_`)})}function vt(t,e){const s=_t(t,e);return s?t.entities?.[s]?.device_id:void 0}function bt(t,e){if(!e)return[];const s=vt(t,e);return function(t){return $t(t).filter(e=>!!t.states[`switch.sf_${e}_outlet_1`])}(t).filter(i=>{if(i===e)return!0;if(!s)return!1;const r=vt(t,i),n=r?t.devices?.[r]:void 0;return n?.via_device_id===s})}function yt(t,e){if(!t||!e)return"";const s=_t(t,e);if(!s)return"";const i=t.entities?.[s]?.device_id,r=i?t.devices?.[i]:void 0;if(r)return r.name_by_user||r.name||"";const n=(t.states[s].attributes.friendly_name||"").match(/^(SF .+? [0-9A-Fa-f]{4})\b/);return n?n[1]:""}const xt=[["temperature","Air Temp","mdi:thermometer"],["humidity","Air Humi","mdi:water-percent"],["vpd","VPD","mdi:water-opacity"],["co2","CO2","mdi:molecule-co2"],["ppfd","PPFD","mdi:white-balance-sunny"],["soil_avg_temperature","Soil Temp","mdi:thermometer"],["soil_avg_moisture","Moisture","mdi:water"],["soil_avg_ec","Soil EC","mdi:flash"]],wt=[["light_1","Light 1","mdi:lightbulb"],["light_2","Light 2","mdi:lightbulb"]],At=[["fan","Fan","mdi:fan"],["blower","Blower","mdi:weather-windy"]],St=[["heater","Heater","mdi:radiator"],["humidifier","Humidifier","mdi:air-humidifier"],["dehumidifier","Dehumidifier","mdi:air-humidifier-off"]],Et=[["Temperature","env_temp_day","env_temp_night","env_temp_deadband","mdi:thermometer"],["Humidity","env_humi_day","env_humi_night","env_humi_deadband","mdi:water-percent"],["CO2","env_co2_day","env_co2_night","env_co2_deadband","mdi:molecule-co2"]];let Ct=class extends at{constructor(){super(...arguments),this.tab="overview"}setConfig(t){if(!t.panel)throw new Error('spider-farmer-card: "panel" is required (e.g. panel: dp1)');this.config=t,this.tab="config"===t.default_tab?"config":"overview"}getCardSize(){return 8}static getConfigElement(){return document.createElement("spider-farmer-card-editor")}static getStubConfig(t){const e=(t?mt(t):[])[0]||"dp1",s=t?bt(t,e):[];return{type:"custom:spider-farmer-card",panel:e,...s.length?{outlets:s}:{}}}eid(t,e){return`${t}.sf_${this.config.panel}_${e}`}get(t){return this.hass?.states[t]}accent(){return this.config.accent||"#ff7a1a"}shouldUpdate(t){return t.has("config")||t.has("hass")||t.has("tab")}renderParam([t,e,s]){const i=this.get(`sensor.sf_${this.config.panel}_${t}`);if(!i)return q;const r=i.attributes.unit_of_measurement||"",n=this.hass?.formatEntityState?this.hass.formatEntityState(i).replace(r,"").trim():i.state;return F`
      <div class="tile">
      ...
      ${Et.map(([e,s,i,r,n])=>this.get(`number.sf_${t}_${s}`)?F`
        ...
        </div>`)}return e}renderConfig(){return F`${this.renderEnv()}${this.renderOutlets()}`}render(){if(!this.hass||!this.config)return q;const t=this.hasConfig(),e=t?this.tab:"overview",s=yt(this.hass,this.config.panel);return F`
      <ha-card>
        <div class="header">
  ...
  `,t([pt({attribute:!1})],Ct.prototype,"hass",void 0),t([ut()],Ct.prototype,"config",void 0),t([ut()],Ct.prototype,"tab",void 0),Ct=t([ct("spider-farmer-card")],Ct);let Pt=class extends at{constructor(){super(...arguments),this._config={type:"custom:spider-farmer-card"}}setConfig(t){this._config={...t}}_emit(t){this._config=t,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:t},bubbles:!0,composed:!0}))}_panelChanged(t){const e=t.target.value,s={...this._config};e?s.panel=e:delete s.panel,this._emit(s)}_titleChanged(t){const e=t.target.value.trim(),s={...this._config};e?s.title=e:delete s.title,this._emit(s)}_tabChanged(t){const e=t.target.value;this._emit({...this._config,default_tab:e})}_outletToggled(t,e){const s=e.target.checked,i=new Set(this._config.outlets??[]);s?i.add(t):i.delete(t);const r=[...i].sort(),n={...this._config};r.length?n.outlets=r:delete n.outlets,this._emit(n)}render(){if(!this.hass)return q;const t=this._config,e=mt(this.hass),s=bt(this.hass,t.panel),i=t=>{const e=yt(this.hass,t);return e?`${t} — ${e}`:t};return F`
      <div class="form">
        <label class="field">
