/*! spider-farmer-card v0.20.32 | MIT */
...
`;function wt(t){const e=new Set;for(const s of Object.keys(t.states)){const t=bt(s).match(vt);t&&e.add(t[1])}return[...e].sort()}function St(t){return wt(t).filter(e=>e.startsWith("st")&&!!t.states[`switch.sf_${e}_outlet_1`]||Object.keys(t.states).some(t=>{const s=bt(t);return s===`sf_${e}_temperature`||s===`sf_${e}_soil_avg_temperature`||s===`sf_${e}_light_1`||s===`sf_${e}_fan`||s===`sf_${e}_blower`}))}function kt(t,e){const s=`sf_${e}_`,i=Object.keys(t.states);return i.find(t=>bt(t)===`sf_${e}_temperature`)??i.find(t=>{const i=bt(t);return i.startsWith(s)&&!i.startsWith(`sf_${e}_env_`)})}function Ot(t,e){const s=kt(t,e);return s?t.entities?.[s]?.device_id:void 0}function Ct(t,e){if(!e)return[];const s=Ot(t,e);return s?function(t){return wt(t).filter(e=>!!t.states[`switch.sf_${e}_outlet_1`])}(t).filter(i=>{if(i===e)return!1;const o=Ot(t,i),a=o?t.devices?.[o]:void 0;return a?.via_device_id===s}):[]}function Mt(t,e){if(!t||!e)return"";const s=kt(t,e);if(!s)return"";const i=t.entities?.[s]?.device_id,o=i?t.devices?.[i]:void 0;if(o)return o.name_by_user||o.name||"";const a=(t.states[s].attributes.friendly_name||"").match(/^(SF .+? [0-9A-Fa-f]{4})\b/);return a?a[1]:""}const Dt=[["temperature","Air Temp","mdi:thermometer"],["humidity","Air Humi","mdi:water-percent"],["vpd","Air VPD","mdi:water-opacity"],["leaf_vpd","Leaf VPD","mdi:leaf"],["co2","CO2","mdi:molecule-co2"],["ppfd","PPFD","mdi:white-balance-sunny"],["soil_avg_temperature","Soil Temp","mdi:thermometer"],["soil_avg_moisture","Moisture","mdi:water"],["soil_avg_ec","Soil EC","mdi:flash"]],Nt={temperature:"temp",humidity:"humi",vpd:"vpd",co2:"co2",ppfd:"ppfd",soil_avg_temperature:"tempSoil",soil_avg_moisture:"humiSoil",soil_avg_ec:"ECSoil"},At={temperature:"temp",humidity:"humi",co2:"co2"},Tt="#ff6b6b",Et="rgba(255,107,107,0.16)",Rt=(t,e=.16)=>{const s=/^#?([0-9a-fA-F]{6})$/.exec((t||"").trim());if(!s)return`rgba(255,107,107,${e})`;const i=parseInt(s[1],16);return`rgba(${i>>16&255},${i>>8&255},${255&i},${e})`},Pt=t=>"string"==typeof t&&/^#[0-9a-fA-F]{6}$/.test(t),Lt=t=>"alarms"===t||"targets"===t||"both"===t,zt=mt,Ft={"Time Slot":["ts_type","ts_start","ts_stop"],Cycle:["cycle_start","cycle_run","cycle_off","cycle_times"],Temperature:["temp_device"],Humidity:["humidity_device"],CO2:["co2_device"],"Drip Irrigation":["drip_soil","drip_avg"],Manual:[]},Bt=mt,It=t=>!!t&&("unavailable"===t.state||"unknown"===t.state),Ht=t=>{const e=(t||"").match(/^(\d{1,2}):(\d{2})$/);if(!e)return null;const s=+e[1],i=+e[2];return s<=23&&i<=59?60*s+i:null},jt=(t,e,s,i,o="")=>V`<div class="save-bar ${o}">
  ...
  <button class="discard-btn" ?disabled=${!e}
    @click=${i}>Discard</button>`)(t,e,s,i)}
        ...
        <div class="tile-label">
          ...
          <span class="tl-right">
            ...
            ${l?V`<ha-icon class="tile-more"
                  icon=${c?"mdi:chevron-up":"mdi:chevron-down"}></ha-icon>`:p?V`<ha-icon class="tile-more"
      ...
      <div class="soil-pop">
        ...
        ${e.map(({name:e,e:s})=>{const i=It(s),o=s.attributes.unit_of_measurement||"",a=this.hass?.formatEntityState?this.hass.formatEntityState(s).replace(o,"").trim():s.state,r="temperature"===t?"tempSoil":"moisture"===t?"humiSoil":"ECSoil",n=i?null:this.readingColor(r,s.state);return V`
      ...
      </div>`}deviceSummaryRow(t){const e=this.deviceSummaryLines(t);return e.length?V`<div class="tile-summary">
      ...
      <div class="params-head">
        ...
        ${s}
      </div>`}renderOverview(){const t=Dt.map(t=>this.renderParam(t)).filter(t=>t!==q),e=this.soilStatsTile(),s=this.overviewDevices();return V`
      ...
      ${this.renderVpd()}
      ${this.applyBar(this.envIds())}`}renderPlan(t){const e=this.accent(),s=`switch.sf_${this.config.panel}_plan_enabled`,i=!!this.get(s),o=t.stages,a=t.progress||{},r=(t,i)=>V`
      <button class="plan-btn ${i?"stop":"start"}"
        ...
        ${o.map(t=>this.renderPlanStage(t,null))}
        ${i?V`<div class="plan-actions">${r("Start Plan",!1)}</div>`:q}`:V`
          <div class="plan-empty">
            ...
            <div class="plan-empty-sub">
              Create a plan in the Spider Farmer app. Once it exists you can start
              and stop it here; while it runs, the manual Environment targets pause.
            </div>
          </div>`;const n=o.find(t=>t.stageId===a.stageId)||o[0],l=Number(a.progress),c=Number.isFinite(l),d=[];return null!=a.planted&&d.push(`${this.fmtNum(a.planted)} planted`),null!=a.remain&&d.push(`${this.fmtNum(a.remain)} left`),null!=a.totalDays&&d.push(`${this.fmtNum(a.totalDays)} total`),V`
      <div class="plan-banner">
        <ha-icon icon="mdi:sprout" style="color:${e}"></ha-icon>
        <div style="flex:1">
          <div class="plan-banner-title">${n&&n.label||"Planting plan active"}</div>
          <div class="plan-banner-sub">
            ${d.length?d.join(" · ")+" days":`${o.length} stage${1===o.length?"":"s"} · managed by the controller`}
          </div>
        </div>
        ${c?V`<div class="plan-pct" style="color:${e}">${Math.round(l)}%</div>`:q}
      </div>
      ${c?V`<div class="plan-bar"><div class="plan-bar-fill"
            style="width:${Math.max(0,Math.min(100,l))}%;background:${e}"></div></div>`:q}
      <div class="section-label">Stages</div>
      ${o.map(t=>this.renderPlanStage(t,a.stageId))}
      ...
      </div>`}renderPlanStage(t,e){const s=this.accent(),i=null!=e&&t.stageId===e,o=(t,e,s,i="")=>V`
      ...
      </div>`,a=null!=t.co2_day||null!=t.co2_night;return V`
      <div class="plan-stage ${i?"current":""}"
        style=${i?`border-color:${s}`:""}>
        <div class="plan-stage-head">
          <span class="plan-stage-dot" style="background:${s}"></span>
          <span class="plan-stage-name">${t.label||"Stage"}</span>
          ${i?V`<span class="plan-stage-badge" style="color:${s};border-color:${s}">Current</span>`:q}
        </div>
        <div class="plan-stage-grid">
          ${o("Temp",this.fmtTemp(t.temp_night),this.fmtTemp(t.temp_day))}
          ${o("Humidity",t.humi_night,t.humi_day,"%")}
          ${a?o("CO₂",t.co2_night,t.co2_day,""):q}
        </div>
      ...
      ${t.map(t=>{const s=Mt(this.hass,t)||(t.startsWith("st")?"S-Station":`${t.toUpperCase()} Power Strip`);return V`
      ...
      ${o.length?V`
        ...
        style=${e===t?`color:${s};border-color:${s}`:""}
        @click=${()=>this.tab=t}>${i}</button>`,o=Mt(this.hass,this.config.panel);return V`
      ...
      <div class="seg-row">
        ${N(s,"off","No color","mdi:circle-off-outline",()=>D({mode:"off"}))}
        ${N(s,"tile","Tile color","mdi:square-rounded",()=>D({mode:"tile"}))}
        ${N(s,"text","Text color","mdi:format-color-text",()=>D({mode:"text"}))}
      </div>
      <div class="color-row">
        ${E("Above max",o,t=>D({hi:t}))}
        ${E("Below min",a,t=>D({lo:t}))}
        ${E("Near edge",n,t=>D({warn:t}))}
      </div>
      ...
      <div class="seg-row">
        ${N(i,"off","No color","mdi:circle-off-outline",()=>D({modeIn:"off"}))}
        ${N(i,"tile","Tile color","mdi:square-rounded",()=>D({modeIn:"tile"}))}
        ${N(i,"text","Text color","mdi:format-color-text",()=>D({modeIn:"text"}))}
      </div>
      <div class="color-row">
        ${E("In range",r,t=>D({in:t}))}
      </div>
      ...
      ${y?V`
            ...
            <div class="set-note">
              ...
              <span style="color:${b}">Manual</span>,
              <span style="color:${$}">Scheduled</span>,
              <span style="color:${_}">Environment</span>,
              <span style="color:${x}">Drip</span>. Off outlets stay neutral.
            </div>
            <div class="seg-row">
              ${N(v,"off","No color","mdi:circle-off-outline",()=>D({omode:"off"}))}
              ${N(v,"tile","Tile color","mdi:square-rounded",()=>D({omode:"tile"}))}
              ${N(v,"text","Text color","mdi:format-color-text",()=>D({omode:"text"}))}
            </div>
            <div class="color-row">
              ${E("Manual",b,t=>D({ocManual:t}))}
              ${E("Scheduled",$,t=>D({ocSched:t}))}
            </div>
            <div class="color-row">
              ${E("Environment",_,t=>D({ocEnv:t}))}
              ${E("Drip",x,t=>D({ocDrip:t}))}
            </div>`:q}
      ...
      ${C?V`
            ...
            <div class="seg-row">
              ${N(w,"off","No color","mdi:circle-off-outline",()=>D({dmode:"off"}))}
              ${N(w,"tile","Tile color","mdi:square-rounded",()=>D({dmode:"tile"}))}
              ${N(w,"text","Text color","mdi:format-color-text",()=>D({dmode:"text"}))}
            </div>
            <div class="color-row">
              ${E("Manual",S,t=>D({dcManual:t}))}
              ${E("Scheduled",k,t=>D({dcSched:t}))}
              ${E("Auto",O,t=>D({dcAuto:t}))}
            </div>`:q}
  ...
  `,t([ut({attribute:!1})],Kt.prototype,"hass",void 0),t([ft()],Kt.prototype,"_config",void 0);const Jt=/^sf_(se\d+)_light$/;function Zt(t){const e=new Set;for(const s of Object.keys(t.states)){if(!s.startsWith("light."))continue;const t=bt(s).match(Jt);t&&e.add(t[1])}return[...e].sort((t,e)=>Number(t.replace(/\D/g,""))-Number(e.replace(/\D/g,"")))}class Xt extends ct{constructor(){super(...arguments),this.draft=null,this.ctlDraft={}}setConfig(t){this.config=t}getCardSize(){return 7}static getStubConfig(t){const e=t?Zt(t):[];return{type:"custom:spider-light-card",...e[0]?{light:e[0]}:{}}}accent(){return this.config.accent||mt}seSlot(){return this.config.light||(this.hass?Zt(this.hass)[0]:"")||"se1"}get(t){return this.hass?.states[t]}shouldUpdate(t){return t.has("config")||t.has("hass")||t.has("draft")||t.has("ctlDraft")}cur(t,e){return t in this.ctlDraft?this.ctlDraft[t]:e}stageCtl(t,e){this.ctlDraft={...this.ctlDraft,[t]:e}}isDirty(){return Object.keys(this.ctlDraft).length>0||null!==this.draft}applyAll(t){const e=this.ctlDraft,s=`light.sf_${t}_light`;if("bri"in e){const t=Number(e.bri);t>0?this.hass?.callService("light","turn_on",{entity_id:s,brightness_pct:t}):this.hass?.callService("light","turn_off",{entity_id:s})}"mode"in e&&this.hass?.callService("select","select_option",{entity_id:`select.sf_${t}_mode`,option:e.mode});for(const t of Object.keys(e))t.includes(".")&&(t.startsWith("number.")?this.hass?.callService("number","set_value",{entity_id:t,value:Number(e[t])}):t.startsWith("text.")&&this.hass?.callService("text","set_value",{entity_id:t,value:e[t]}));if("power"in e){const t="on"===e.power;t&&"bri"in e||this.hass?.callService("light",t?"turn_on":"turn_off",{entity_id:s})}this.draft&&this.saveSchedule(t),this.ctlDraft={}}discardAll(){Object.keys(this.ctlDraft).length&&(this.ctlDraft={}),null!==this.draft&&(this.draft=null)}render(){if(!this.hass||!this.config)return q;const t=this.seSlot(),e=this.get(`light.sf_${t}_light`);if(!e)return V`<ha-card>
      ...
      </ha-card>`;const s="on"===e.state,i=s?Math.max(0,Math.min(100,Math.round((e.attributes.brightness??0)/255*100))):0,o=this.get(`select.sf_${t}_mode`),a=this.cur("mode",o?.state??""),r="bri"in this.ctlDraft,n="on"===this.cur("power",s?"on":"off"),l=r?Number(this.ctlDraft.bri):n?i:0,c=r?l>0:n,d=Mt(this.hass,t),h=this.accent(),p=l/100,[u,f]=$t(100,100,78,135+270*p);return V`
      <ha-card>
        <div class="header">
        ...
        <div class="gauge">
          <svg viewBox="0 0 200 190" aria-hidden="true">
            ...
            ${c&&p>0?W`<path d=${_t(100,100,78,0,p)} fill="none"
                  stroke-linecap="round" stroke=${h} stroke-width="15"></path>`:q}
            ${c?W`<circle cx=${u.toFixed(2)} cy=${f.toFixed(2)} r="10"
