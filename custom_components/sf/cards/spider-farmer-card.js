/*! spider-farmer-card v0.17.18 | MIT */
...
`;function St(t){const e=new Set;for(const s of Object.keys(t.states)){const t=bt(s).match(vt);t&&e.add(t[1])}return[...e].sort()}function kt(t){return St(t).filter(e=>Object.keys(t.states).some(t=>{const s=bt(t);return s===`sf_${e}_temperature`||s===`sf_${e}_soil_avg_temperature`||s===`sf_${e}_light_1`||s===`sf_${e}_fan`||s===`sf_${e}_blower`}))}function Ot(t,e){const s=`sf_${e}_`,i=Object.keys(t.states);return i.find(t=>bt(t)===`sf_${e}_temperature`)??i.find(t=>{const i=bt(t);return i.startsWith(s)&&!i.startsWith(`sf_${e}_env_`)})}function At(t,e){const s=Ot(t,e);return s?t.entities?.[s]?.device_id:void 0}function Ct(t,e){if(!e)return[];const s=At(t,e);return function(t){return St(t).filter(e=>!!t.states[`switch.sf_${e}_outlet_1`])}(t).filter(i=>{if(i===e)return!0;if(!s)return!1;const r=At(t,i),o=r?t.devices?.[r]:void 0;return o?.via_device_id===s})}function Rt(t,e){if(!t||!e)return"";const s=Ot(t,e);if(!s)return"";const i=t.entities?.[s]?.device_id,r=i?t.devices?.[i]:void 0;if(r)return r.name_by_user||r.name||"";const o=(t.states[s].attributes.friendly_name||"").match(/^(SF .+? [0-9A-Fa-f]{4})\b/);return o?o[1]:""}const Et=[["temperature","Air Temp","mdi:thermometer"],["humidity","Air Humi","mdi:water-percent"],["vpd","VPD","mdi:water-opacity"],["co2","CO2","mdi:molecule-co2"],["ppfd","PPFD","mdi:white-balance-sunny"],["soil_avg_temperature","Soil Temp","mdi:thermometer"],["soil_avg_moisture","Moisture","mdi:water"],["soil_avg_ec","Soil EC","mdi:flash"]],Mt={temperature:"temp",humidity:"humi",vpd:"vpd",co2:"co2",ppfd:"ppfd",soil_avg_temperature:"tempSoil",soil_avg_moisture:"humiSoil",soil_avg_ec:"ECSoil"},Nt="#ff6b6b",Pt="#45b6ff",Dt="rgba(255,107,107,0.16)",Tt=t=>!!t&&("unavailable"===t.state||"unknown"===t.state),zt=[["light_1","Light 1","mdi:lightbulb"],["light_2","Light 2","mdi:lightbulb"]],Bt=[["fan","Fan","mdi:fan"],["blower","Blower","mdi:weather-windy"]],Ft=[["heater","Heater","mdi:radiator"],["humidifier","Humidifier","mdi:air-humidifier"],["dehumidifier","Dehumidifier","mdi:air-humidifier-off"]],Ut=[["Temperature","env_temp_day","env_temp_night","env_temp_deadband","mdi:thermometer"],["Humidity","env_humi_day","env_humi_night","env_humi_deadband","mdi:water-percent"],["CO2","env_co2_day","env_co2_night","env_co2_deadband","mdi:molecule-co2"]];class jt extends ct{constructor(){super(...arguments),this.tab="overview",this.colorMode="off",this._colorSynced=!1,this.alertsDraft=null,this.soilOpen=null,this.soilAllOpen=!1,this.deviceOpen=null,this.outletOpen=null,this.draft={},this.modePick={},this.outletDraft={},this.logDate=null,this.logDev="all",this.logType="all"}setConfig(t){if(!t.panel)throw new Error('spider-farmer-card: "panel" is required (e.g. panel: dp1)');this.config=t;const e=t.default_tab;this.tab="environment"===e||"config"===e?"env":"outlets"===e?"outlets":"calibration"===e||"cali"===e?"cali":"alerts"===e?"alerts":"log"===e?"log":"overview";const s=t.alarm_colors;let i="tile"===s||"text"===s?s:"off";try{const e=localStorage.getItem(`sf-colors-${t.panel}`);"off"!==e&&"tile"!==e&&"text"!==e||(i=e)}catch{}this.colorMode=i,this._colorSynced=!1}serverColorMode(){const t=this.get(`sensor.sf_${this.config.panel}_alarm_settings`)?.attributes.card_options,e=t?.colors;return"off"===e||"tile"===e||"text"===e?e:null}setColorMode(t){this.colorMode=t,this._colorSynced=!0;try{localStorage.setItem(`sf-colors-${this.config.panel}`,t)}catch{}const e=`sensor.sf_${this.config.panel}_alarm_settings`;this.get(e)&&this.hass?.callService("sf","set_card_option",{entity_id:e,key:"colors",value:t})}alarmStatus(t,e){if("off"===this.colorMode)return null;const s=Mt[t];if(!s)return null;const i=this.alertsSettings();if(!i)return null;const r=[...i.climate||[],...i.substrate||[]].find(t=>t&&t.key===s);if(!r||!r.enabled)return null;const o=parseFloat(e);if(!Number.isFinite(o))return null;const a=Number(r.max),n=Number(r.min);return Number.isFinite(a)&&o>a?"above":Number.isFinite(n)&&o<n?"below":null}getCardSize(){return 8}static getConfigElement(){return document.createElement("spider-farmer-card-editor")}static getStubConfig(t){const e=(t?kt(t):[])[0]||"dp1",s=t?Ct(t,e):[];return{type:"custom:spider-farmer-card",panel:e,...s.length?{outlets:s}:{}}}eid(t,e){return`${t}.sf_${this.config.panel}_${e}`}get(t){return this.hass?.states[t]}accent(){return this.config.accent||gt}shouldUpdate(t){return t.has("config")||t.has("hass")||t.has("tab")||t.has("soilOpen")||t.has("soilAllOpen")||t.has("outletDraft")||t.has("alertsDraft")||t.has("deviceOpen")||t.has("outletOpen")||t.has("draft")||t.has("modePick")||t.has("logDate")||t.has("logDev")||t.has("logType")||t.has("colorMode")}willUpdate(t){if(!this._colorSynced){const t=this.serverColorMode();if(t){this.colorMode=t,this._colorSynced=!0;try{localStorage.setItem(`sf-colors-${this.config.panel}`,t)}catch{}}}if(t.has("hass")&&Object.keys(this.modePick).length){let t=null;for(const[e,s]of Object.entries(this.modePick))this.get(e)?.state===s&&(t=t??{...this.modePick},delete t[e]);t&&(this.modePick=t)}}renderParam([t,e,s]){const i=this.get(`sensor.sf_${this.config.panel}_${t}`);if(!i)return q;const r=i.attributes.unit_of_measurement||"",o=t.startsWith("soil_avg_")&&Tt(i),a=o?"Offline":this.hass?.formatEntityState?this.hass.formatEntityState(i).replace(r,"").trim():i.state,n=t.startsWith("soil_avg_")?t.slice(9):null,l=!!n&&this.soilProbeRows(n).length>1,c=l&&this.soilOpen===n,d=this.alarmStatus(t,i.state),p="above"===d?Nt:"below"===d?Pt:"";let h=c?`box-shadow:inset 0 0 0 1px ${this.accent()}`:"",u="";if(o)h=`background:${Dt};box-shadow:inset 0 0 0 1px ${Nt}`,u=`color:${Nt}`;else if(d&&"text"===this.colorMode)u=`color:${p}`;else if(d&&"tile"===this.colorMode){h=`background:${"above"===d?Dt:"rgba(69,182,255,0.16)"};box-shadow:inset 0 0 0 1px ${p}`}return I`
      ...
      ${Ut.map(([e,s,i,r,o])=>this.get(`number.sf_${t}_${s}`)?I`
          <div class="env-row">
      ...
      </div>`:q}toggleOutlet(t){this.outletOpen=t}outletNums(t){const e=[];for(let s=1;s<=10;s++)this.get(`select.sf_${t}_outlet_${s}_mode`)&&e.push(s);return e}renderOutlets(){const t=this.outletSlots().filter(t=>this.outletNums(t).length>0);if(!t.length)return q;const e=this.outletOpen?this.outletOpen.slice(0,this.outletOpen.lastIndexOf("_")):null;return I`
      ${t.map(t=>{const s=Rt(this.hass,t)||`${t.toUpperCase()} Power Strip`;return I`
          <div class="section-label">${s}</div>
          <div class="grid">
            ${this.outletNums(t).map(e=>this.outletTile(t,e))}
          </div>
          ${e===t?this.renderOutletPop():q}`})}`}outletTile(t,e){const s="on"===this.get(`switch.sf_${t}_outlet_${e}`)?.state,i=this.get(`select.sf_${t}_outlet_${e}_mode`)?.state??"",r=this.outletKey(t,e),o=this.outletOpen===r,a=this.accent();return I`
      <div class="tile clickable ${o?"active":""}"
        style=${o?`box-shadow:inset 0 0 0 1px ${a}`:q}
        role="button" aria-expanded=${o?"true":"false"}
        @click=${()=>this.toggleOutlet(o?null:r)}>
        ...
        <div class="tile-val" style=${s?`color:${a}`:q}>${s?"On":"Off"}</div>
        <div class="tile-sub">${i}</div>
      </div>`}renderOutletPop(){const t=this.outletOpen;if(!t)return q;const e=t.lastIndexOf("_");if(e<0)return q;const s=t.slice(0,e),i=Number(t.slice(e+1));if(!s||!Number.isFinite(i))return q;const r=`select.sf_${s}_outlet_${i}_mode`;if(!this.get(r))return q;const o=`switch.sf_${s}_outlet_${i}`,a=this.get(o),n=`sf_${s}_outlet_${i}_`,l="Time Slot"===(this.get(r)?.state??"")&&!!this.get(`sensor.${n}ts_schedule`),c=Object.keys(this.hass?.states??{}).filter(t=>{const e=t.split(".")[1]??"";return!(!e.startsWith(n)||e===`${n}mode`)&&(e!==`${n}ts_schedule`&&(!l||e!==`${n}ts_type`&&e!==`${n}ts_start`&&e!==`${n}ts_stop`))}).sort();return I`
      <div class="soil-pop" style="--sf-accent:${this.accent()}">
        <div class="soil-pop-head">
          <span>Outlet ${i}</span>
          <ha-icon icon="mdi:close" role="button" aria-label="Close"
        ...
        ${a?this.renderControl(o,"Power"):q}
        ${this.renderControl(r,"Mode")}
        ${c.map(t=>this.renderControl(t))}
        ${l?this.renderOutletSchedule(s,i):q}
      </div>`}outletKey(t,e){return`${t}_${e}`}outletPeriods(t,e){const s=this.outletDraft[this.outletKey(t,e)];if(s)return s;const i=this.get(`sensor.sf_${t}_outlet_${e}_ts_schedule`)?.attributes.periods;return Array.isArray(i)?i:[]}editOutlet(t,e,s){const i=this.outletKey(t,e),r=this.outletDraft[i]??this.outletPeriods(t,e),o=JSON.parse(JSON.stringify(r));s(o),this.outletDraft={...this.outletDraft,[i]:o}}clearOutletDraft(t,e){const s=this.outletKey(t,e),i={...this.outletDraft};delete i[s],this.outletDraft=i}saveOutlet(t,e){const s=this.outletDraft[this.outletKey(t,e)];s&&(this.hass?.callService("sf","set_outlet_schedule",{entity_id:`select.sf_${t}_outlet_${e}_mode`,periods:s}),this.clearOutletDraft(t,e))}renderOutletSchedule(t,e){const s=this.outletPeriods(t,e),i=!!this.outletDraft[this.outletKey(t,e)],r=this.accent();return I`
      <div class="ts-editor">
        ${s.map((s,i)=>I`
          <div class="period">
      ...
      ${r.length?I`
        ...
        @click=${()=>this.tab=t}>${e}</button>`,c=Rt(this.hass,this.config.panel);return I`
      <ha-card>
        <div class="header">
      ...
      <div class="set-note">
        ...
        metrics whose alarm is switched on in the Alerts tab. Saved to the
  ...
  `,t([ut({attribute:!1})],jt.prototype,"hass",void 0),t([ft()],jt.prototype,"config",void 0),t([ft()],jt.prototype,"tab",void 0),t([ft()],jt.prototype,"colorMode",void 0),t([ft()],jt.prototype,"alertsDraft",void 0),t([ft()],jt.prototype,"soilOpen",void 0),t([ft()],jt.prototype,"soilAllOpen",void 0),t([ft()],jt.prototype,"deviceOpen",void 0),t([ft()],jt.prototype,"outletOpen",void 0),t([ft()],jt.prototype,"draft",void 0),t([ft()],jt.prototype,"modePick",void 0),t([ft()],jt.prototype,"outletDraft",void 0),t([ft()],jt.prototype,"logDate",void 0),t([ft()],jt.prototype,"logDev",void 0),t([ft()],jt.prototype,"logType",void 0);class Ht extends ct{constructor(){super(...arguments),this._config={type:"custom:spider-farmer-card"}}setConfig(t){this._config={...t}}_emit(t){this._config=t,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:t},bubbles:!0,composed:!0}))}_panelChanged(t){const e=t.target.value,s={...this._config};e?s.panel=e:delete s.panel,this._emit(s)}_titleChanged(t){const e=t.target.value.trim(),s={...this._config};e?s.title=e:delete s.title,this._emit(s)}_tabChanged(t){const e=t.target.value;this._emit({...this._config,default_tab:e})}_outletToggled(t,e){const s=e.target.checked,i=new Set(this._config.outlets??[]);s?i.add(t):i.delete(t);const r=[...i].sort(),o={...this._config};r.length?o.outlets=r:delete o.outlets,this._emit(o)}render(){if(!this.hass)return q;const t=this._config,e=t.default_tab,s=kt(this.hass),i=Ct(this.hass,t.panel),r=t=>{const e=Rt(this.hass,t);return e?`${t} — ${e}`:t};return I`
      <div class="form">
        <label class="field">
  ...
  `,t([ut({attribute:!1})],Ht.prototype,"hass",void 0),t([ft()],Ht.prototype,"_config",void 0);const Lt=/^sf_(se\d+)_light$/;function Wt(t){const e=new Set;for(const s of Object.keys(t.states)){if(!s.startsWith("light."))continue;const t=bt(s).match(Lt);t&&e.add(t[1])}return[...e].sort((t,e)=>Number(t.replace(/\D/g,""))-Number(e.replace(/\D/g,"")))}class It extends ct{constructor(){super(...arguments),this.draft=null}setConfig(t){this.config=t}getCardSize(){return 7}static getStubConfig(t){const e=t?Wt(t):[];return{type:"custom:spider-light-card",...e[0]?{light:e[0]}:{}}}accent(){return this.config.accent||gt}seSlot(){return this.config.light||(this.hass?Wt(this.hass)[0]:"")||"se1"}get(t){return this.hass?.states[t]}shouldUpdate(t){return t.has("config")||t.has("hass")||t.has("draft")}setBrightness(t){const e=`light.sf_${this.seSlot()}_light`;t<=0?this.hass?.callService("light","turn_off",{entity_id:e}):this.hass?.callService("light","turn_on",{entity_id:e,brightness_pct:t})}toggle(){this.hass?.callService("light","toggle",{entity_id:`light.sf_${this.seSlot()}_light`})}setMode(t){this.hass?.callService("select","select_option",{entity_id:`select.sf_${this.seSlot()}_mode`,option:t})}render(){if(!this.hass||!this.config)return q;const t=this.seSlot(),e=this.get(`light.sf_${t}_light`);if(!e)return I`<ha-card>
      ...
      </ha-card>`;const s="on"===e.state,i=s?Math.max(0,Math.min(100,Math.round((e.attributes.brightness??0)/255*100))):0,r=this.get(`select.sf_${t}_mode`),o=r?.state??"",a=Rt(this.hass,t),n=this.accent(),l=i/100,[c,d]=$t(100,100,78,135+270*l);return I`
      <ha-card>
        <div class="header">
    ...
    .save { color: #fff; }
    .discard { background: var(--secondary-background-color); color: var(--primary-text-color); }
  `,t([ut({attribute:!1})],It.prototype,"hass",void 0),t([ft()],It.prototype,"config",void 0),t([ft()],It.prototype,"draft",void 0),customElements.get("spider-farmer-card")||customElements.define("spider-farmer-card",jt),customElements.get("spider-farmer-card-editor")||customElements.define("spider-farmer-card-editor",Ht),customElements.get("spider-light-card")||customElements.define("spider-light-card",It),window.customCards=window.customCards||[],window.customCards.push({type:"spider-farmer-card",name:"Spider Farmer Card",description:"Tent overview + config for the Spider Farmer Bridge integration",preview:!0,documentationURL:"https://github.com/cobragt2000/spider_farmer_bridge"}),window.customCards.push({type:"spider-light-card",name:"Spider Light Card",description:"Brightness dial, mode, and schedule for a Spider Farmer SE-series light",preview:!0,documentationURL:"https://github.com/cobragt2000/spider_farmer_bridge"}),console.info("%c SPIDER-FARMER-CARD %c v0.17.18 ","color:#fff;background:#ff7a1a;border-radius:3px 0 0 3px;padding:2px 4px","color:#ff7a1a;background:#222;border-radius:0 3px 3px 0;padding:2px 4px"),(()=>{const t=["spider-farmer-card","spider-light-card"],e=new Set([...t,...t.map(t=>`custom:${t}`)]),s=()=>{const t=[["spider-farmer-card",jt],["spider-farmer-card-editor",Ht],["spider-light-card",It]];for(const[e,s]of t)if(!customElements.get(e))try{customElements.define(e,s)}catch{}},i=()=>{let t=0;for(const s of(()=>{const t=[],e=new Set,s=i=>{if(!i||e.has(i))return;e.add(i);let r=[];try{r=i.querySelectorAll("hui-error-card")}catch{return}r.forEach(e=>t.push(e));let o=[];try{o=i.querySelectorAll("*")}catch{return}o.forEach(t=>{const e=t.shadowRoot;e&&s(e)})};return s(document),t})()){const i=s._config||{},r=i.origConfig&&i.origConfig.type||i.type||"";e.has(r)&&(s.dispatchEvent(new CustomEvent("ll-rebuild",{bubbles:!0,composed:!0})),t++)}return t};let r=0;const o=()=>{s(),i(),++r<12&&setTimeout(o,250)},a=()=>{s(),o()};"complete"===document.readyState?a():window.addEventListener("load",a,{once:!0})})();export{jt as SpiderFarmerCard,Ht as SpiderFarmerCardEditor,It as SpiderLightCard};
